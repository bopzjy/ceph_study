...
struct hobject_t {
  ...
  snapid_t snap;
private:
// hash和key不能同时设置，hash值一般设置为pg的id值
  uint32_t hash;
  bool max;
...
struct ghobject_t {
  hobject_t hobj;
  /*    用于记录对象版本号。
   *    在EC模式，写操作需要区分写前后两个版本的object, 写操作保存对象的上一个版本（generation）的对象
   *    当EC写失败时，可以rollback到上一个版本
   */
  gen_t generation;
  /*    
   *    用来标识对象所在的osd在EC类型的PG中的序号
   *    在EC模式下，每个osd在PG中的序号在数据恢复时非常关键
   *    如果是Replicate, 该字段设置为NO_SHARD(-1), 即对于replicate是没用的
   */
  shard_id_t shard_id;
